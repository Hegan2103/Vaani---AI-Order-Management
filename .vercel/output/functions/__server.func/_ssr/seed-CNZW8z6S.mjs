//#region node_modules/.nitro/vite/services/ssr/assets/seed-CNZW8z6S.js
function item(id, name, aliases, unit, stock, listPrice) {
	return {
		id,
		name,
		aliases,
...
var catalogs = {
	pharmaceutical: [
		...
		item("p6", "Betadine 50ml", [
			"betadine",
			"povidone",
			"बेटाडीन"
		], "bottle", 40, 95),
		item("p7", "Amoxicillin 500", [
			"amox",
			"amoxicillin",
			"एमॉक्स"
		], "strip", 60, 78),
		item("p8", "Cough Syrup 100ml", [
			"cough syrup",
			"खांसी की दवाई",
			"corex"
		], "bottle", 35, 110)
	...
	electrical: [
		item("e1", "Copper Wire 1.5mm", [
			"1.5 mm wire",
			"1.5mm",
			"house wire",
			"तार 1.5"
		], "metre", 1200, 28),
		item("e2", "Copper Wire 2.5mm", [
			"2.5 mm wire",
			"2.5mm",
			"power wire",
			"तार 2.5"
	...
	hardware: [
		item("h1", "Wood Screw 2 inch", [
			"screw",
			"screws",
			"कांटा",
			"पेच"
		], "box", 90, 85),
		item("h2", "Door Hinge 4 inch", [
			"hinge",
			"hinges",
			"कब्जा"
		], "pair", 70, 120),
		item("h3", "Padlock 50mm", [
			"lock",
			"talla",
			"ताला"
		], "piece", 45, 160),
		item("h4", "Nails 2 inch", [
			"nail",
			"nails",
			"कील"
		], "kg", 60, 95),
	...
	construction: [
		item("c1", "OPC Cement 50kg", [
			"cement",
			"cement bag",
			"सीमेंट"
		], "bag", 400, 380),
		item("c2", "TMT Bar 12mm", [
			"steel",
			"tmt",
			"rod",
			"sariya",
		...
		item("c5", "Vitrified Tile 2x2", [
			"tile",
			"tiles",
			"टाइल"
		], "box", 120, 780),
		item("c6", "Wall Putty 40kg", [
			"putty",
			"wall putty",
			"पुट्टी"
		], "bag", 90, 620)
	...
	electronics: [
		item("x1", "USB-C Cable 1m", [
			"usb cable",
			"type c",
			"केबल"
		], "piece", 150, 149),
		item("x2", "20W Fast Charger", [
			"charger",
			"adapter",
			"चार्जर"
		], "piece", 60, 499),
		item("x3", "Wired Earphones", [
			"earphones",
			"earphone",
			"इयरफोन"
		], "piece", 80, 299),
		item("x4", "32GB Pen Drive", [
			"pendrive",
			"pen drive",
			"usb",
			"पेनड्राइव"
...
var DEFAULT_CONTACTS = VENDORS.map((v) => ({
	id: `c-${v.id}`,
	name: v.shop,
	phone: v.phone,
	vendorId: v.id,
...
var SAMPLE_UTTERANCES = [
	...
	"Aata 25 kg, daal 10 kg, chini ka price batao",
...
function inboxIdForUser(userId) {
	return `u-${userId.replace(/^u[:\-]/, "")}`;
}
function line(id, kind, productName, quantity, unit, raw) {
	return {
		id,
		kind,
		raw,
...
function demoIncomingFor(vendorId) {
	...
	const byIndustry = {
		pharmaceutical: [{
			...
			lines: [line("d4", "order", "Amoxicillin 500", 8, "strip", "Amoxicillin 500 8 strips"), line("d5", "inquiry", "cough syrup 100 ml", null, "bottle", "cough syrup 100 ml rate")]
		}],
		grocery: [{
			...
			lines: [line("d4", "order", "Tel", 10, "litre", "Tel 10 litre"), line("d5", "order", "namak", 25, "kg", "namak 25 kg")]
		}],
		electrical: [{
	...
	return (byIndustry[vendor.industry] ?? byIndustry.grocery).map((row, i) => ({
		id: `in-${vendorId}-${i + 1}`,
		vendorId,
		...
		createdAt: (/* @__PURE__ */ new Date(now - (i + 1) * 47 * 60 * 1e3)).toISOString(),
		status: "sent",
		lines: row.lines.map((l) => ({
			...l,
			id: `in-${vendorId}-${i + 1}-${l.id}`
		})),
		orderCopy: null,
		notes: ""
	}));
}
function demoIncomingForIndustry(industry, inboxId) {
...
//#endregion
export { VENDORS as a, phoneDigits as c, SAMPLE_UTTERANCES as i, phonesMatch as l, INDUSTRY_LABEL as n, demoIncomingForIndustry as o, LANGUAGES as r, inboxIdForUser as s, DEFAULT_CONTACTS as t };
