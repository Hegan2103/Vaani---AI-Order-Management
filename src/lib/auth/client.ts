...
/** Vaani uses its own phone login. Platform WhatsApp/Google/X stay off. */
...
export function storeBearerToken(token: string | null): void {
...
export async function signInGoogle(callbackURL = "/"): Promise<void> {
  ...
  if (error) {
    const detail = [error.message, "code" in error ? String((error as { code?: string }).code) : ""]
      .filter(Boolean)
      .join(" — ");
