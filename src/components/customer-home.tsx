...
import { INDUSTRY_LABEL, VENDORS, allIndustrySamples, formatInPhone, samplesFor } from "@/lib/vaani/seed";
import { useVaani, vendorById, vendorForPhone } from "@/lib/vaani/store";
  ...
  return (
    <>
      <section className="mb-8 grid gap-6 md:grid-cols-[1.2fr_0.8fr]">
        <div>
          <p className="text-xs font-medium uppercase tracking-[0.16em] text-muted">For every shop</p>
          ...
          <p className="mt-3 max-w-prose text-sm text-muted">
            {customerPhone ? (
              <>
                Signed in as <span className="font-medium text-ink">{formatInPhone(customerPhone)}</span>.
              </>
            ) : null}
          </p>
          <ol className="mt-3 max-w-prose list-decimal space-y-1.5 pl-5 text-sm text-muted">
            <li>Save your shop card, then pull your phone directory.</li>
            <li>Tap a vendor who is on Vaani.</li>
            <li>
              Speak <span className="text-ink">product + quantity</span> to order, or{" "}
              <span className="text-ink">product + cost</span> to ask a price.
            </li>
            <li>Vendor accepts, rejects, or quotes. Then they finalise the order copy.</li>
          </ol>
        </div>
        <ShopCard
          extra={
          ...
          return (
            <li key={c.id} className="flex items-center justify-between gap-3 px-4 py-3">
              ...
              {v ? (
                ...
                <span className="text-xs text-subtle">Not on Vaani</span>
              )}
            </li>
          );
        })}
      ...
      <p className="mt-4 text-xs text-subtle">
        Say product + quantity to order. Say product + cost/rate/kitna to inquire. Hindi, English, and
        other Indian languages work. Spoken names stay as spoken.
      </p>
      ...
      {industry === "all" ? (
        <div className="mt-4 space-y-3">
          {allIndustrySamples().map((row) => (
            <div key={row.id}>
              <p className="text-xs font-medium text-muted">Example — {row.label}</p>
        ...
        <div className="mt-4">
          <p className="text-xs font-medium text-muted">Example — {INDUSTRY_LABEL[industry]}</p>
          <div className="mt-1 flex flex-wrap gap-2">
      ...
      {tickets.length > 0 ? (
        <section className="mt-10">
          <h2 className="mb-3 font-display text-2xl tracking-tight">Your requests</h2>
                ...
                {groupByDate(rows).map((g) => (
                  <div key={g.key}>
                    ...
                    <ul className="space-y-2">
                      {g.tickets.map((t) => {
                        const vend = vendorById(t.vendorId);
                        return (
                          <li key={t.id}>
                            <Link
                              to="/ticket/$ticketId"
                              ...
                              onClick={() => void navigate({ to: "/ticket/$ticketId", params: { ticketId: t.id } })}
                            >
                              <div className="min-w-0">
                                <p className="truncate font-medium">{vend?.shop ?? t.customerName}</p>
                                <p className="truncate text-xs text-muted">
                                  {t.lines.length} lines · {new Date(t.createdAt).toLocaleString("en-IN")}
                                </p>
                              </div>
        ...
        <p className="mt-10 flex items-center gap-2 text-sm text-muted">
          ...
          {customerPhone
            ? `No voice orders yet for ${formatInPhone(customerPhone)}. Pick a vendor and speak.`
            : "No voice orders yet. Pick a vendor and speak."}
        </p>
      )}
    </>
  );
